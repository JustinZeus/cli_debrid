...
def add_wanted_items(media_items_batch: List[Dict[str, Any]], versions_input):
    ...
    try:
        ...
        do_not_add_watched = get_setting(
            "Debug", "do_not_add_plex_watch_history_items_to_queue", False
        )
        ...
        version_summary = {"movies": {}, "episodes": {}}

        enable_granular_versions = get_setting(
            "Debug", "enable_granular_version_additions", False
        )
        ...
        if movie_imdb_ids:
            ...
            for i in range(0, len(movie_imdb_list), batch_size):
                ...
                for row in rows:
                    ...
                    existing_movies[movie_id].append(
                        (strip_version(row["version"]), row["state"])
                    )
        ...
        for item in media_items_batch:
            ...
            if do_not_add_watched and watch_history_conn:
                if item_type == "movie":
                    if imdb_id or tmdb_id:
                        query_wh = (
                            "SELECT 1 FROM watch_history WHERE type = 'movie' AND "
                        )
                        params_wh = []
                        conditions_wh = []
                        if imdb_id:
                            conditions_wh.append("imdb_id = ?")
                            params_wh.append(imdb_id)
                        if tmdb_id:
                            conditions_wh.append("tmdb_id = ?")
                            params_wh.append(tmdb_id)
                        if conditions_wh:
                            query_wh += " OR ".join(conditions_wh)
                            if watch_history_conn.execute(
                                query_wh, params_wh
                            ).fetchone():
            ...
            if item_type == "movie":
                ...
                if (
                    tmdb_id
                    and tmdb_id in existing_movies
                    and (not imdb_id or imdb_id != tmdb_id)
                ):
                ...
                season_number_check = item.get("season_number")
                episode_number_check = item.get("episode_number")
                existing_versions_states_check = []
                imdb_key_check = None
                tmdb_key_check = None
                if imdb_id:
                    ...
                    if imdb_key_check in existing_episodes:
                        existing_versions_states_check.extend(
                            existing_episodes[imdb_key_check]
                        )
                if tmdb_id:
                    tmdb_key_check = (
                        str(tmdb_id),
                    ...
                    if tmdb_key_check in existing_episodes and (
                        not imdb_key_check or imdb_key_check != tmdb_key_check
                    ):
                        existing_versions_states_check.extend(
                            existing_episodes[tmdb_key_check]
                        )
                for _, state in existing_versions_states_check:
                    if state == "Blacklisted":
                        is_blacklisted_in_db = True
            ...
            if item_type == "movie":
                skip = False
                media_id_vs = imdb_id or tmdb_id
                existing_versions_set_vs = set()
                existing_states_set_vs = set()
                if imdb_id and imdb_id in existing_movies:
                    for version_vs, state_vs in existing_movies[imdb_id]:
                        existing_versions_set_vs.add(version_vs)
                        existing_states_set_vs.add(state_vs)
                if (
                    tmdb_id
                    and tmdb_id in existing_movies
                    and (not imdb_id or imdb_id != tmdb_id)
                ...
                if not enable_granular_versions:
                    if existing_versions_set_vs:
                        ...
                        if media_id_vs not in version_summary["movies"]:
                            version_summary["movies"][media_id_vs] = {
                                "existing": existing_versions_set_vs,
                                "added": set(),
                                "title": normalized_title,
                                "states": existing_states_set_vs,
                            }
                else:
                    new_versions_vs = {
                        v: enabled
                        for v, enabled in versions.items()
                        if strip_version(v) not in existing_versions_set_vs
                    }
                    if new_versions_vs:
                        if media_id_vs not in version_summary["movies"]:
                            version_summary["movies"][media_id_vs] = {
                                "existing": existing_versions_set_vs,
                                "added": set(new_versions_vs.keys()),
                                "title": normalized_title,
                                "states": existing_states_set_vs,
                            }
                        else:
                            version_summary["movies"][media_id_vs]["added"].update(
                                new_versions_vs.keys()
                            )
                ...
                season_number_vs = item.get("season_number")
                episode_number_vs = item.get("episode_number")
                skip = False
                media_id_vs = imdb_id or tmdb_id
                ...
                if tmdb_id:
                    ...
                    if tmdb_key_vs in existing_episodes and (
                        not imdb_key_vs or imdb_key_vs != tmdb_key_vs
                    ):
                ...
                if not enable_granular_versions:
                    if existing_versions_set_vs:
                        skip = True
                        if imdb_key_vs and imdb_key_vs in existing_episodes:
                            skip_stats["existing_episode_imdb"] += 1
                        if (
                            tmdb_key_vs
                            and tmdb_key_vs in existing_episodes
                            and (not imdb_key_vs or imdb_key_vs != tmdb_key_vs)
                        ):
                            skip_stats["existing_episode_tmdb"] += 1
                        if episode_key_vs not in version_summary["episodes"]:
                            version_summary["episodes"][episode_key_vs] = {
                                "existing": existing_versions_set_vs,
                                "added": set(),
                                "title": normalized_title,
                                "states": existing_states_set_vs,
                    ...
                    if new_versions_vs:
                        if episode_key_vs not in version_summary["episodes"]:
                            ...
                            version_summary["episodes"][episode_key_vs]["added"].update(
                                new_versions_vs.keys()
                            )
                        item["versions_to_add"] = new_versions_vs
                    else:
        ...
        for item in media_items_batch:
            ...
            is_item_blacklisted = is_blacklisted(
                item.get("imdb_id", ""), season_number_for_blacklist
            ) or is_blacklisted(item.get("tmdb_id", ""), season_number_for_blacklist)
            ...
            if not item.get("tmdb_id"):
                ...
                tmdb_id_meta, media_type_meta = get_tmdb_id_and_media_type(
                    item["imdb_id"]
                )
                if tmdb_id_meta:
                    ...
                    logging.warning(
                        f"Unable to retrieve tmdb_id for {item.get('title', 'Unknown')} (IMDb ID: {item['imdb_id']})"
                    )
            ...
            if (
                item_type == "episode"
                and content_source
                and content_source.startswith("Collected_")
            ):
                if content_source in content_sources:
                    source_config = content_sources[content_source]
                    monitor_mode = source_config.get(
                        "monitor_mode", "Monitor All Episodes"
                    )
                    ...
                    if monitor_mode != "Monitor All Episodes":
                        ...
                        if not release_date_str:
                            logging.warning(
                                f"MONITOR_MODE_SKIP (Missing Date): Episode '{normalized_title}' from source '{content_source}'. monitor_mode: {monitor_mode}."
                            )
                        ...
                        try:
                            ...
                            logging.warning(
                                f"MONITOR_MODE_SKIP (Invalid Date Format): Episode '{normalized_title}' from source '{content_source}' due to invalid release date format '{release_date_str}'. monitor_mode: {monitor_mode}."
                            )
                    ...
                    logging.warning(
                        f"Content source '{content_source}' for item '{normalized_title}' not found in configuration. Skipping monitor_mode check for this item."
                    )
            ...
            for version, enabled in versions_to_use.items():
                ...
                version_config = (
                    config.get("Scraping", {}).get("versions", {}).get(version, {})
                )
                ...
                if item_type == "movie":
                    ...
                    release_date_str = item.get("release_date")
                    check_trakt = False
                    trakt_early_releases_enabled = get_setting(
                        "Scraping", "trakt_early_releases", False
                    )

                    if trakt_early_releases_enabled and imdb_id:
                        if (
                            not release_date_str
                            or release_date_str.lower() == "unknown"
                        ):
                            check_trakt = True
                        else:
                            try:
                    ...
                    if check_trakt:
                        logging.info(
                            f"Checking Trakt early release lists for movie: {normalized_title} ({imdb_id})"
                        )
                        try:
                            trakt_search_results = fetch_items_from_trakt(
                                f"/search/imdb/{imdb_id}"
                            )
                            if (
                                trakt_search_results
                                and isinstance(trakt_search_results, list)
                                and len(trakt_search_results) > 0
                            ):
                                if "movie" in trakt_search_results[
                                    0
                                ] and trakt_search_results[0]["movie"].get(
                                    "ids", {}
                                ).get(
                                    "trakt"
                                ):
                                    trakt_id = str(
                                        trakt_search_results[0]["movie"]["ids"]["trakt"]
                                    )
                                    trakt_lists = fetch_items_from_trakt(
                                        f"/movies/{trakt_id}/lists/personal/popular"
                                    )
                                    if trakt_lists:
                                        for trakt_list in trakt_lists:
                                            if re.search(
                                                r"(latest|new).*?(releases)",
                                                trakt_list.get("name", ""),
                                                re.IGNORECASE,
                                            ):
                                        ...
                                        logging.warning(
                                            f"Failed to fetch Trakt lists for movie {trakt_id}"
                                        )
                                else:
                                    logging.warning(
                                        f"Could not extract Trakt ID from search results for {imdb_id}"
                                    )
                            else:
                                logging.info(
                                    f"No Trakt search results found for {imdb_id}"
                                )
                        except Exception as e:
                            logging.error(
                                f"Error checking Trakt early release for {imdb_id}: {str(e)}"
                            )
                    ...
                    movie_data = (
                        ...
                        item.get("theatrical_release_date"),
                        early_release_flag,
                    )
                    ...
                    if get_setting("Debug", "allow_partial_overseerr_requests"):
                        initial_state = (
                            "Wanted"
                            if item.get("is_requested_season", True)
                            else "Blacklisted"
                        )
                    blacklisted_date = (
                        datetime.now(timezone.utc)
                        if initial_state == "Blacklisted"
                        else None
                    )
                    ...
                    episode_data = (
                        ...
                        item.get("country", "").lower(),
                        blacklisted_date,
                        item.get("requested_season", False),
                        item.get("content_source"),
                        item.get("content_source_detail"),
                    )
        ...
        if show_titles_to_potentially_update:
            logging.debug(
                f"Processing {len(show_titles_to_potentially_update)} unique show title update candidates."
            )
        ...
        if enable_granular_versions:
            ...
            if version_summary["movies"]:
                ...
                for media_id, info in version_summary["movies"].items():
                    ...
                    if info["existing"]:
                        skip_report.append(
                            f"    - Existing versions: {sorted(info['existing'])}"
                        )
                    if info["added"]:
                        skip_report.append(
                            f"    - Added versions: {sorted(info['added'])}"
                        )
                    if not info["added"]:
                        skip_report.append(
                            "    - No new versions added (all requested versions exist)"
                        )
                    if "Blacklisted" in info.get("states", set()):
                        skip_report.append(
                            "    - Note: At least one existing version is Blacklisted (addition was skipped earlier)"
                        )

            # Episodes summary
            if version_summary["episodes"]:
                skip_report.append("\nEpisodes:")
                for (media_id, season, episode), info in version_summary[
                    "episodes"
                ].items():
                    id_type = "IMDb" if str(media_id).startswith("tt") else "TMDb"
                    skip_report.append(
                        f"  {info['title']} S{season:02d}E{episode:02d} ({id_type} ID: {media_id}):"
                    )
                    if info["existing"]:
                        skip_report.append(
                            f"    - Existing versions: {sorted(info['existing'])}"
            ...
            skipped_movie_count = (
                skip_stats["existing_movie_imdb"] + skip_stats["existing_movie_tmdb"]
            )
            if skipped_movie_count > 0:
                skip_report.append(
                    f"- {skipped_movie_count} movies skipped because at least one version already exists"
                )
            skipped_episode_count = (
                skip_stats["existing_episode_imdb"]
                + skip_stats["existing_episode_tmdb"]
            )
            if skipped_episode_count > 0:
                skip_report.append(
                    f"- {skipped_episode_count} episodes skipped because at least one version already exists"
                )
        ...
        if skip_stats["missing_ids"] > 0:
            skip_report.append(
                f"- {skip_stats['missing_ids']} items skipped due to missing IMDb/TMDb IDs"
            )
        if skip_stats["blacklisted"] > 0:
            skip_report.append(
                f"- {skip_stats['blacklisted']} items skipped due to blacklist"
            )
        if skip_stats["already_watched"] > 0:
            skip_report.append(
                f"- {skip_stats['already_watched']} items skipped due to watch history"
            )
        if skip_stats["media_type_mismatch"] > 0:
            skip_report.append(
                f"- {skip_stats['media_type_mismatch']} items skipped due to media type mismatch"
            )
        if skip_stats["anime_filter"] > 0:
            skip_report.append(
                f"- {skip_stats['anime_filter']} version additions skipped due to anime filter mode"
            )
        if skip_stats["trakt_error"] > 0:
            skip_report.append(
                f"- {skip_stats['trakt_error']} items skipped Trakt check due to API errors"
            )  # Report Trakt errors

        # Add new monitor_mode skip reasons to the report
        if skip_stats.get("monitor_mode_future_skip", 0) > 0:
            skip_report.append(
                f"- {skip_stats['monitor_mode_future_skip']} episodes skipped by 'Monitor Future Episodes' mode"
            )
        if skip_stats.get("monitor_mode_recent_skip", 0) > 0:
            skip_report.append(
                f"- {skip_stats['monitor_mode_recent_skip']} episodes skipped by 'Monitor Recent (90 Days) and Future' mode"
            )
        if skip_stats.get("monitor_mode_no_date", 0) > 0:
            skip_report.append(
                f"- {skip_stats['monitor_mode_no_date']} episodes skipped by monitor mode due to missing release date"
            )
        if skip_stats.get("monitor_mode_invalid_date", 0) > 0:
            skip_report.append(
                f"- {skip_stats['monitor_mode_invalid_date']} episodes skipped by monitor mode due to invalid release date format"
            )

        if skip_report:
            logging.info(
                "Wanted items processing complete. Skip summary:\n"
                + "\n".join(skip_report)
            )
        logging.info(
            f"Final stats - Added: {items_added}, Updated: {items_updated}, Total Skipped: {items_skipped}"
        )
    except Exception as e:
...
def update_show_title(
    conn, imdb_id: str = None, tmdb_id: str = None, new_title: str = None
) -> bool:
    """
    ...
    update_params = [
        ...
        datetime.now(timezone.utc),
    ] + params  # Renamed params to update_params
    conn.execute(update_query, update_params)

    logging.info(
        f"Updated show title from '{existing_title}' to '{normalized_new_title}' for {row['record_count']} records (IMDb: {imdb_id}, TMDb: {tmdb_id})"
    )  # Added IDs for clarity
    return True
