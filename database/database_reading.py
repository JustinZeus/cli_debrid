    ...
    def wrapper(*args, **kwargs):
        ...
        if tracemalloc_available and tracemalloc_enabled:
            if tracemalloc.is_tracing():
                try:
                    ...
                    logging.error(
                        f"[Tracemalloc DB] Error getting memory before {func.__name__}: {e_mem}"
                    )
        ...
        if tracemalloc_available and tracemalloc_enabled and tracemalloc.is_tracing():
            try:
                ...
                logging.error(
                    f"[Tracemalloc DB] Error getting memory after {func.__name__}: {e_mem}"
                )
...
def search_movies(search_term):
    ...
    cursor = conn.execute(
        'SELECT * FROM media_items WHERE type = "movie" AND title LIKE ?',
        (f"%{search_term}%",),
    )
...
def search_tv_shows(search_term):
    conn = get_db_connection()
    cursor = conn.execute(
        'SELECT * FROM media_items WHERE type = "episode" AND (title LIKE ? OR episode_title LIKE ?)',
...
def get_all_media_items(
    state=None, media_type=None, imdb_id=None, tmdb_id=None, limit: Optional[int] = None
):
    """
...
def get_movie_runtime(tmdb_id):
    ...
    try:
        cursor = conn.execute(
            'SELECT runtime FROM media_items WHERE tmdb_id = ? AND type = "movie"',
            (tmdb_id,),
        )
        result = cursor.fetchone()
...
def get_episode_runtime(tmdb_id):
    ...
    try:
        ...
        logging.error(
            f"Error retrieving episode runtime (TMDB ID: {tmdb_id}): {str(e)}"
        )
        return None
    finally:
        if conn:
...
def get_all_season_episode_counts(tmdb_id):
    ...
    try:
        ...
        logging.error(
            f"Error retrieving season episode counts (TMDB ID: {tmdb_id}): {str(e)}"
        )
        return {}
    finally:
        conn.close()
...
def get_media_items_presence_batch(imdb_ids: List[str]) -> Dict[str, str]:
    """
    Batch check presence for multiple IMDB IDs.

    Returns:
        Dict mapping imdb_id to status: "Collected", "Upgrading", "Wanted", "Blacklisted", etc.
        Items not found in DB will not be in the returned dict.
    """
    if not imdb_ids:
        return {}
    ...
    conn = get_db_connection()
    try:
        # Use the same aggregation logic as get_media_item_presence_overall
        # but handle multiple IDs in one query
        placeholders = ",".join("?" * len(imdb_ids))

        # Get all distinct states per imdb_id
        query = f"""
            SELECT imdb_id, state 
            FROM media_items 
        ...
        # Group states by imdb_id
        states_by_id = {}
        for row in cursor.fetchall():
            imdb_id = row["imdb_id"]
            state = row["state"]

            if imdb_id not in states_by_id:
                states_by_id[imdb_id] = set()
            states_by_id[imdb_id].add(state)

        # Apply aggregation logic (same as get_media_item_presence_overall)
        result = {}
        for imdb_id, states in states_by_id.items():
            if "Blacklisted" in states:
                result[imdb_id] = "Blacklisted"
            elif "Collected" in states:
                result[imdb_id] = "Collected" if len(states) == 1 else "Partial"
            else:
                # Return first state found
                result[imdb_id] = next(iter(states))

        logging.debug(
            f"Batch presence check: queried {len(imdb_ids)} IDs, found {len(result)} in DB"
        )
        return result

    except Exception as e:
        logging.error(f"Batch presence query failed: {e}")
        return {}
    finally:
        conn.close()
        ...
        if key in seen_dates:
            ...
            if key not in [group["key"] for group in duplicate_groups]:
                duplicate_groups.append(
                    {"key": key, "episodes": [seen_dates[key], episode]}
                )
            else:
    ...
    try:
        ...
        for group in duplicate_groups:
            sorted_episodes = sorted(
                group["episodes"], key=lambda x: x["season_number"]
            )
        ...
        if ids_to_delete:
            ...
            logging.info(
                f"Deleted {len(ids_to_delete)} duplicate episodes for tmdb_id {tmdb_id}"
            )
        ...
        logging.error(
            f"Error cleaning up duplicate episodes (TMDB ID: {tmdb_id}): {str(e)}"
        )
        return 0
    finally:
        conn.close()
    ...
    try:
        ...
        if row:
            ...
            if not video.get("location_on_disk") and video.get("filled_by_file"):
                logging.warning(
                    f"No location_on_disk for video {video_id}, falling back to filled_by_file"
                )
            return video
...
def get_media_country_code(tmdb_id: str) -> str:
    ...
    try:
        cursor = conn.execute(
            "SELECT country FROM media_items WHERE tmdb_id = ? LIMIT 1", (tmdb_id,)
        )
        result = cursor.fetchone()
...
def get_episode_details(imdb_id: str, season: int, episode: int) -> dict:
    ...
    try:
        ...
        logging.error(
            f"Error retrieving episode details (IMDB ID: {imdb_id}, S{season:02d}E{episode:02d}): {str(e)}"
        )
        return None
    finally:
        conn.close()
    ...
    try:
        ...
        if result and result["imdb_aliases"]:
            try:
                ...
                logging.error(
                    f"Error parsing IMDB aliases JSON for {imdb_id}: {str(e)}"
                )
                return [imdb_id]
        ...
        if item:
            ...
            if item_dict.get("type") == "episode":
                ...
                item_dict.setdefault(
                    "show_title", None
                )  # Add show_title default (may need to derive this)
                item_dict.setdefault(
                    "show_year", None
                )  # Add show_year default (may need to derive this)
            return item_dict
        else:
...
def check_existing_media_item(
    item_details: Dict, target_version: str, target_states: List[str]
) -> bool:
    """
    ...
    try:
        base_query = (
            "SELECT 1 FROM media_items WHERE version = ? AND state IN ({})".format(
        ...
        if imdb_id:
            ...
            if tmdb_id:
                ...
                logging.warning(
                    "Cannot check for existing item without imdb_id or tmdb_id."
                )
        ...
        if item_type == "episode":
            ...
            if season_number is not None and episode_number is not None:
                ...
                logging.warning(
                    f"Cannot check for existing episode without season/episode number for ID {imdb_id or tmdb_id}."
                )
            ...
            logging.warning(
                f"Unknown item type '{item_type}' for checking existing media."
            )
            return False
        ...
        logging.error(
            f"Error checking for existing media item (Version: {target_version}, States: {target_states}): {str(e)}"
        )
...
def get_wake_count(item_id: int) -> int:
    ...
    try:
        cursor = conn.execute(
            "SELECT wake_count FROM media_items WHERE id = ?", (item_id,)
        )
        result = cursor.fetchone()
        # Return the count, default to 0 if NULL or not found
        return (
            result["wake_count"] if result and result["wake_count"] is not None else 0
        )
    except Exception as e:
...
def get_show_episode_identifiers_from_db(
        ...
        for row in cursor:
            ...
            if s_num is not None and e_num is not None:
                try:
                    ...
                    logging.warning(
                        f"Skipping invalid non-integer season/episode number pair ({s_num}, {e_num}) from DB for show {id_field}={id_value}"
                    )
            # No need for else, IS NOT NULL handles it

        logging.debug(
            f"Found {len(identifiers)} unique S/E pairs in DB for {id_field}={id_value}"
        )
        return identifiers
    except Exception as e:
        logging.error(
            f"Error retrieving episode identifiers for show {id_field}={id_value}: {str(e)}"
        )
...
def get_media_item_ids(imdb_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    try:
            ...
            if imdb_id_from_row in results:
                ...
                if item_type == "movie":
                    ...
                    if state == "Collected" and version:
                        results[imdb_id_from_row]["collected_movie_qualities"].add(
                            version
                        )
                    ...
                    if season_num is not None and episode_num is not None:
                        try:
                            ...
                            logging.warning(
                                f"Skipping invalid non-integer season/episode number pair ({season_num}, {episode_num}) from DB for IMDb {imdb_id_from_row}"
                            )
        ...
        logging.info(
            f"Retrieved DB states/identifiers for {len(rows)} rows corresponding to {len(imdb_ids)} requested IMDb IDs."
        )
        return results
...
def check_item_exists_by_directory_name(item_directory_name: str) -> bool:
    ...
    try:
        ...
        for row in rows:
            ...
            if (
                filled_by_title
                and normalize_string_for_comparison(filled_by_title)
                == normalized_search
            ):
                logging.debug(
                    f"Found existing item in DB with matching filled_by_title: {item_directory_name}"
                )
                return True

            if (
                real_debrid_title
                and normalize_string_for_comparison(real_debrid_title)
                == normalized_search
            ):
                logging.debug(
                    f"Found existing item in DB with matching real_debrid_original_title: {item_directory_name}"
                )
                return True

        logging.debug(
            f"No existing items found in DB with matching directory/title name: {item_directory_name}"
        )
        return False

    except Exception as e:
        logging.error(
            f"Error checking items by directory/title name ('{item_directory_name}'): {str(e)}"
        )
        ...
        for row in rows:
            ...
            if original_path:
                ...
                if normalized_path.startswith(normalized_prefix):
                    logging.debug(
                        f"Found existing item in DB whose original_path_for_symlink starts with: {path_prefix}"
                    )
                    return True

        logging.debug(
            f"No existing item found in DB whose original_path_for_symlink starts with: {path_prefix}"
        )
        return False

    except Exception as e:
        logging.error(
            f"Error checking items by original_path_for_symlink prefix ('{original_dir_path}'): {str(e)}"
        )
        return False  # Assume not found on error to avoid blocking unnecessarily
    finally:
        if conn:
        ...
        for row in rows:
            ...
            if original_path:
                ...
                if normalized_segment in normalized_path:
                    logging.debug(
                        f"Found existing item in DB where original_path_for_symlink CONTAINS segment: {path_segment}"
                    )
                    return True

        logging.debug(
            f"No existing items found in DB where original_path_for_symlink CONTAINS segment: {path_segment}"
        )
        return False

    except Exception as e:
        logging.error(
            f"Error checking items by original_path_for_symlink containing segment ('{path_segment}'): {str(e)}"
        )
        return False  # Assume not found on error
    finally:
        if conn:
...
def get_distinct_library_shows(letter: Optional[str] = None) -> List[Dict[str, str]]:
    ...
    try:
        ...
        if letter:
            ...
            if letter_upper == "#":
                filter_conditions.append(
                    "NOT (SUBSTR(UPPER(rst.title), 1, 1) BETWEEN 'A' AND 'Z')"
                )
                ...
                logging.warning(
                    f"Invalid letter '{letter}' provided for filtering shows. Returning all shows or based on other criteria."
                )
        ...
        logging.debug(
            f"Found {len(shows)} distinct shows in the library (filter: '{letter}', new query)."
        )
        return shows
    except Exception as e:
        logging.error(
            f"Error retrieving distinct library shows (filter: '{letter}', new query): {str(e)}"
        )
        return []
    finally:
        if conn:
...
def get_collected_episodes_count(imdb_id: str, version_name: str) -> int:
    ...
    try:
        ...
        logging.debug(
            f"DB Collected Count for IMDb: {imdb_id}, Version: {version_name} (after asterisk trim) -> {count}"
        )
        return count
    except Exception as e:
        logging.error(
            f"Error counting collected episodes for IMDb ID {imdb_id} (version: {version_name}): {str(e)}"
        )
        return 0
    finally:
        if conn:
...
def get_collected_episode_numbers(
    imdb_id: str, version_name: str
) -> Set[Tuple[int, int]]:
    """
    ...
    try:
        ...
        for row in cursor.fetchall():
            try:
                ...
                logging.warning(
                    f"Skipping invalid non-integer season/episode number pair ({row['season_number']}, {row['episode_number']}) from DB for show {imdb_id}"
                )

        return collected_episodes
    except Exception as e:
        logging.error(
            f"Error getting collected episode numbers for {imdb_id} (version: {version_name}): {e}",
            exc_info=True,
        )
        return set()
    finally:
...
def get_media_item_presence_overall(
    imdb_id: str | None = None, tmdb_id: str | None = None
) -> str:
    ...
    try:
        ...
        for f in filenames:
            ...
            if basename in db_basenames:
                logging.info(
                    f"Found matching file in DB for IMDb ID {imdb_id}: {basename}"
                )
                return True
...
@trace_memory_usage
def get_distinct_imdb_ids(
    states: Optional[List[str]] = None, media_type: Optional[str] = None
) -> List[str]:
...
def get_season_year(
    imdb_id: str = None, tmdb_id: str = None, season_number: int = None
) -> Optional[int]:
    """
    ...
    if not (imdb_id or tmdb_id) or season_number is None:
        logging.warning(
            "get_season_year requires either imdb_id or tmdb_id, and season_number"
        )
        return None
        ...
        if result and result["earliest_date"]:
            try:
                ...
                if "-" in date_str:
                    ...
                    year = int(date_str)
                logging.debug(
                    f"Found season year for {id_field}={id_value}, Season {season_number}: {year}"
                )
                return year
            except (ValueError, IndexError) as e:
                logging.warning(
                    f"Invalid date format in season year lookup: {result['earliest_date']} - {e}"
                )

        logging.debug(
            f"No valid season year found for {id_field}={id_value}, Season {season_number}"
        )
        return None

    except Exception as e:
        logging.error(
            f"Error retrieving season year for {id_field}={id_value}, Season {season_number}: {str(e)}"
        )
        return None
    finally:
        if conn:
